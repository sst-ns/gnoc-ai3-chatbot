...
const ViewSettings = (props) => {
  const { onClose, open, selectedList } = props;
  ...
  const getData = async () => {
    ...
    setState(selectedList, body);
  };

  const setState = (data, policyList) => {
    let selectedPolicy = [],
      selectedGraph = [],
      selectedAccounts = [];
    data?.selectedValues?.policy_name.map((el) => {
      if (policyList.includes(el)) {
        selectedPolicy.push(el);
      }
    });

    data?.selectedValues?.accounts.map((el) => {
      if (context.AccountIDs.includes(el)) {
        selectedAccounts.push(el);
      }
    });

    if (data.selectedValues.view === "graph") {
      selectedGraph.push(
        data.selectedValues.action.replace(/(?:^|\s|[-"'([{])+\S/g, (c) =>
          c.toUpperCase()
        )
      );
    }
    ...
    setValue((prev) => ({
      ...prev,
      timeRange: {
        startDate: moment(data.selectedValues.timerange.start_date),
        endDate: moment(data.selectedValues.timerange.end_date),
      },
      selectedInterval: data.selectedValues.interval,
      selectedTab: 0,
      selectedView:
        data.selectedValues.view === "table"
          ? data.selectedValues.action.replace(/(?:^|\s|[-"'([{])+\S/g, (c) =>
              c.toUpperCase()
            )
          : data.selectedValues.view.replace(/(?:^|\s|[-"'([{])+\S/g, (c) =>
              c.toUpperCase()
            ),
      ...
      allAccountsSelected: selectedAccounts.length > 0 ? false : true,
      allPolicySelected: selectedPolicy.length > 0 ? false : true,
    }));
  };
    ...
    >
      ...
      {configPopupData.loading ? (
        ...
        <DialogContent sx={{ margin: "1rem 0.2rem" }}>
          ...
          <Box
            sx={{ padding: "1.5rem 0rem", marginTop: "3rem" }}
            variant="outlined"
          >
            {configPopupData.selectedTab === 0 ? (
              ...
              <Box display="grid" gap="2rem">
                ...
                {configPopupData.policyList.length &&
      ...
      <DialogActions sx={{ boxShadow: "0px 0px 0px 1px lightgray" }}>
        <Button
          ...
          disabled={
            ...
            (configPopupData.selectedView === "Graph" &&
              configPopupData.selectedGraph.length === 1 &&
              configPopupData?.selectedInterval?.length > 0)
              ? false
              : true
          }
